...
PyObject* resizeJPEG(PyObject *self, PyObject *args) {
    ...
    for (int t = 0; t < numThreads; ++t) {
        ...
        threads[t] = new DecoderThread((PyObject*)pyListSrc, start_img, end_img, tgtImgSize, cropToSquare, warpToSquare);
        threads[t]->start();
    }
...
DecoderThread::DecoderThread(PyObject* py_list_src, int start_img, int end_img, int target_size, bool crop_to_square, bool warp_to_square)
: Thread(true), _py_list_src(py_list_src), _start_img(start_img), _end_img(end_img), _target_size(target_size), _crop_to_square(crop_to_square), _warp_to_square(warp_to_square) {
...
void DecoderThread::makeJPEG(int idx) {
     ...
     */
    if (_warp_to_square) {
    	double scale_factor_h = _target_size / double(decoded_mat.size().height);
    	double scale_factor_w = _target_size / double(decoded_mat.size().width);
    	double scale_factor = std::min(scale_factor_h, scale_factor_w);

    	int new_height = _target_size;
    	int new_width = _target_size;
